...
export class DynamoDbService {
    ...
    public async createTable(
        ...
        try {
            this.logger.info(`Service: ${this.createTable.name}`);

            let params = {
                AttributeDefinitions: attributeDefinitions ? attributeDefinitions : null,
                KeySchema: keySchema ? keySchema : null,
                ProvisionedThroughput: {
                    ReadCapacityUnits: readCapacityUnits ? readCapacityUnits : DYNAMODB.DEFAULTS.CREATE.READ_CAPACITY_UNITS,
                    WriteCapacityUnits: writeCapacityUnits ? writeCapacityUnits : DYNAMODB.DEFAULTS.CREATE.WRITE_CAPACITY_UNITS
                },
    ...
    public async listTables(limit: any) {
        try {
            this.logger.info(`Service: ${this.listTables.name}`);
    ...
    public async describeTable(tableName: string) {
        try {
            this.logger.info(`Service: ${this.describeTable.name}`);
    ...
    public async putItem(tableName: string, item: any): Promise<Payload> {
        try {
            this.logger.info(`Service: ${this.putItem.name}`);
    ...
    public async getItem(tableName: any, key: any, projectionExpression?: any) {
        try {
            this.logger.info(`Service: ${this.getItem.name}`);
    ...
    public async scan(
        ...
        try {
            this.logger.info(`Service: ${this.scan.name}`);

            let params = {
                FilterExpression: filterExpression ? filterExpression : null,
                ExpressionAttributeValues: expressionAttributeValues ? Converter.marshall(expressionAttributeValues) : null,
                ProjectionExpression: projectionExpression ? projectionExpression : null,
                TableName: tableName ? tableName : null,
                Limit: limit ? limit : null,
    ...
    public async query(
        ...
        try {
            this.logger.info(`Service: ${this.query.name}`);
    ...
    public async batchWriteItems(tableName: string, params: any) {
        try {
            this.logger.info(`Service: ${this.batchWriteItems.name}`);
    ...
    public async updateItem(
        ...
        try {
            this.logger.info(`Service: ${this.updateItem.name}`);
    ...
    public async getScannedCount(
        ...
        try {
            this.logger.info(`Service: ${this.getScannedCount.name}`);
    ...
    public async getCount(
        ...
        try {
            this.logger.info(`Service: ${this.getCount.name}`);
    ...
    public async scanAll(tableName: string, filterExpression?: string, expressionAttributeValues?: any) {
        try {
            this.logger.info(`Service: ${this.scanAll.name}`);
            ...
            let scanResults: Payload,
                ...
                stopper = true;
            do {
                scanResults = await this.scan(tableName, filterExpression, expressionAttributeValues, null, exclusiveStartKey);
    ...
    public async getScannedCountAll(tableName: string, filterExpression?: string, expressionAttributeValues?: any) {
        try {
            this.logger.info(`Service: ${this.getScannedCountAll.name}`);
            ...
            let scanResults: Payload,
                ...
                stopper = true;
            do {
                scanResults = await this.getScannedCount(tableName, filterExpression, expressionAttributeValues, null, exclusiveStartKey);
                scannedCount += scanResults.data.ScannedCount;
                exclusiveStartKey = Converter.unmarshall(scanResults.data.LastEvaluatedKey);
                if (scanResults.data.LastEvaluatedKey == null) stopper = false;
            } while (stopper);
    ...
    public async getCountAll(tableName: string, filterExpression?: string, expressionAttributeValues?: any) {
        try {
            this.logger.info(`Service: ${this.getCountAll.name}`);
            ...
            let scanResults: Payload,
                ...
                stopper = true;
            do {
                scanResults = await this.getCount(tableName, filterExpression, expressionAttributeValues, null, exclusiveStartKey);
                count += scanResults.data.Count;
                exclusiveStartKey = Converter.unmarshall(scanResults.data.LastEvaluatedKey);
                if (scanResults.data.LastEvaluatedKey == null) stopper = false;
            } while (stopper);
